...
function handleClick(clickedButtonEvent) {
    ...
    const card = {
        ...
        button: clickedButtonEvent.target, // здесь button, который внутри .product
    };

    // 4. получаем текст на кнопке, которая внутри .product
    const btnText = card.button.innerText;
...
function hideMoreText(card) {
    ...
    const desc = card.productName.querySelector('.desc');
    desc.remove();
...
function showMoreText(card) {
    ...
    const text = "Lorem ipsum dolor sit amet consectetur adipisicing elit. Nostrum sapiente suscipit aut? Veniam perspiciatis esse";
    
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.productName.insertAdjacentHTML('afterbegin', `<div class="desc">${ text }</div>`);
