let app = {
    ...
    insertPawns() {
        ...
        const whitePawnsRow = document.querySelectorAll(`[data-rownum="7"]`);
        ...
        const blackPawnsRow = document.querySelectorAll(`[data-rownum="2"]`);
    ...
    insertFiguresOnDesk() {
        ...
        for (let i = 0; i < positions.length; i++) {
            ...
            const cell = document.querySelector(`[data-rownum="${ positions[i].coordRow }"][data-colchar="${ positions[i].coordCol }"]`);

            // 5.2 из массива positions, для текущего объекта по индексу i, передаем название фигуры, затем цвет+"Figure"
            const figure = this.getFigure(positions[i].figure, positions[i].color + "Figure");
    ...
    generateBoard() {
        ...
        for (let i = 0; i < this.config.rows.length; i++) {
            ...
            const row = this.generateRow(rowStartWithColor, this.config.rows[i]);
    ...
    generateRow(startWithColor, rowNum) {
        ...
        for (let i = 0; i < this.config.cols.length; i++) {
            ...
            const field = this.generateField(currentColorClass, rowNum, this.config.cols[i]);
