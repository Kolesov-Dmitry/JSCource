...
function insertPosts() {
    ...
    let postsMarkup = '';
    for (let postIdx = 1; postIdx <= 3; postIdx++) {
        postsMarkup += getPostMarkup(postIdx);
    }
...
function getPostMarkup(counter) {
    /* 2. Сделайте, чтобы в num записывалось текущее время unix (https://mzl.la/3iMz2Jr) плюс counter */
   
    const date = new Date();
    const num = date.getTime() + counter;
